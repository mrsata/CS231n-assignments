...
def batchnorm_backward(dout, cache):
    ...
    x_mean, sqrtvar = x - mean, np.sqrt(var + eps)
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dx_mean1 = dx_normalized / sqrtvar
    dinvert_sqrtvar = np.sum(dx_normalized * x_mean, axis=0)
    dsqrtvar = -dinvert_sqrtvar / sqrtvar ** 2
    dvar = dsqrtvar * 0.5 / sqrtvar
    dsqx_mean2 = dvar * np.ones(x.shape) / x.shape[0]
    dx_mean2 = dsqx_mean2 * 2 * x_mean
    dx1 = dx_mean1 + dx_mean2
    dmean = -np.sum(dx_mean1 + dx_mean2, axis=0)
    dx2 = dmean * np.ones(x.shape) / x.shape[0]
    dx = dx1 + dx2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    out_vanilla, cache = batchnorm_forward(x.transpose(0, 2, 3, 1).reshape(-1, C), gamma, beta, bn_param)
    out = out_vanilla.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dx_vanilla, dgamma, dbeta = batchnorm_backward(dout.transpose(0, 2, 3, 1).reshape(-1, C), cache)
    dx = dx_vanilla.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
